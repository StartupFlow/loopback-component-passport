...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  switch (authType) {
    ...
    case 'saml':
      strategy = new AuthStrategy(_.defaults({
        callbackUrl: callbackURL,
        idpCert: options.certPath ? fs.readFileSync(options.certPath, 'utf-8') : options.cert,
    ...
    case 'openid-connect':
      ...
      strategy = new AuthStrategy({
        ...
        function (req, tokenset, userinfo, done) {
          ...
          console.log('[OIDC][PassportConfigurator][AuthStrategy callback] tokenset', tokenset);
          console.log('[OIDC][PassportConfigurator][AuthStrategy callback] access_token', tokenset.access_token);
          console.log('[OIDC][PassportConfigurator][AuthStrategy callback] id_token', tokenset.id_token);
          console.log('[OIDC][PassportConfigurator][AuthStrategy callback] claims', claims);
          console.log('[OIDC][PassportConfigurator][AuthStrategy callback] userinfo', userinfo);
