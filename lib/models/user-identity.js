...
module.exports = function (UserIdentity) {
  ...
  UserIdentity.login = function (
    ...
    var autoSyncAttributes =
      options.autoSyncAttributes || options.autoSyncAttributes === undefined;
    ...
    userIdentityModel.findOne(
      ...
      function (err, identity) {
          ...
          return identity.updateAttributes(
            ...
            function (err, i) {
              ...
              return identity.user(function (err, user) {
                ...
                if (!err && user) {
                  try {
                    let userObj = (options.profileToUser || profileToUser)(
                      provider,
                      profile,
                      options,
                    );
                    ...
                    for (let attribute in userObj) {
                      const conditionToSyncAttributes = autoSyncAttributes
                        ? attribute !== "password" && userObj[attribute]
                        : attribute !== "password" &&
                          !user[attribute] &&
                          userObj[attribute];
                      if (conditionToSyncAttributes) {
                        newAttributes[attribute] = userObj[attribute];
                      }
                    }

                    user.updateAttributes(newAttributes, options);
                  } catch (err) {
        ...
        userModel.findOrCreate({ where: query }, userObj, options, function (err, user) {
          if (err) {
            return cb(err);
          }
          var date = new Date();
